...
namespace MBHEngine.PathFind.AStar
{
    ...
    public class Planner
    {
        ...
        private void DrawPath(PathNode endPoint)
        {
            ...
            for (Int32 i = 0; i < mClosedNodes.Count; i++)
            {
                DebugShapeDisplay.pInstance.AddAABB(mClosedNodes[i].mTile.mCollisionRect, Color.Red, true);
            ...
            for (Int32 i = 0; i < mOpenNodes.Count; i++)
            {
                DebugShapeDisplay.pInstance.AddAABB(mOpenNodes[i].mTile.mCollisionRect, Color.Green, true);
